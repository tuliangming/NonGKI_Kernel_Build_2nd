...
IMAGE_OUT="/home/skye/bomb/out/arch/arm64/boot/Image"
make -j12 O=/home/skye/bomb/out ARCH=arm64 LLVM=1 LLVM_IAS=1 "${MAKE_OPT[@]}" Image
if [[ ! -f "${IMAGE_OUT}" ]]; then
    echo "ERROR: Kernel Image not found at ${IMAGE_OUT}"
    exit 1
fi
echo "**Build outputs**"
ls /home/skye/bomb/out/arch/arm64/boot || true
echo "**Build outputs**"
cp "${IMAGE_OUT}" /home/skye/bomb/AnyKernel3/Image
